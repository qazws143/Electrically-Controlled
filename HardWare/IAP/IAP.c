...
void IAP_Write(u32 addr, u8 *buf, u32 size)
{
    ...
    for (t = 0; t < size; t += 2) // 循环以两单位为字节进行遍历，这是因为我们需要写入的是半字，这是因为每次给flash编程都需要写入16位数据
    {
        ...
        {
            i = 0;
            STMFLASH_Write(fwaddr, IAP_Buf, 1024); // 进行写入
            fwaddr += 2048;                        // 偏移1024这是为了防止下一次写入操作时，会覆盖上一个块的数据，而512个16位数据，就是512x2个字节，也就是1024
        }
    }
    if (i)
        STMFLASH_Write(fwaddr, IAP_Buf, i); // 循环结束以后，如果ipabuf里边还有剩余数据，就将最后的一些内容字节写进去.
}
